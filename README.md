...
The work was carried out entirely in Microsoft Excel, using formulas and built-in tools (Remove Duplicates, Go To Special, Number Formatting) and conditional formulas (`COUNTIF`, `SUMPRODUCT`, `ISNUMBER`, `TRIM`, `CLEAN`, `EXACT`, `PROPER`, `ROUND`), rather than external scripts, to demonstrate core spreadsheet-based data cleaning competency.
...
Auudit Log
Every change made to the dataset was documented in a dedicated Chamge/Audit Log sheet, recording the Change ID, a description of the action taken, its impact, and its resolution status.
